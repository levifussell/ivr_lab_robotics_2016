...
class DeadReckoningController(Controller):
    ...
    def __init__(self, robot):
        ...
        self.PIDController_position = PIDController(k_proportional=0.3, k_integral=0, k_derivative=0)
        self.PIDController_yaw = PIDController(k_proportional=0.7)
    ...
    def __drive(self, d_position, d_yaw):
        ...
        # if d_yaw > 0:
        self.robot.motorLeft.run_timed(duty_cycle_sp=min(max(d_yaw + d_position, -100), 100), time_sp=50)
        self.robot.motorRight.run_timed(duty_cycle_sp=min(max(-d_yaw + d_position, -100), 100), time_sp=50)
        ...
        #     pos_polarity = -1
        #
        # self.robot.motorLeft.run_timed(duty_cycle_sp=25 * pos_polarity, time_sp=50)
        # self.robot.motorRight.run_timed(duty_cycle_sp=25 * pos_polarity, time_sp=50)
    ...
    def newRelativeTarget(self, d_x, d_y):
        self.position_target = self.position + math.sqrt(d_x*d_x + d_y*d_y) * 20.385

        angle = math.atan2(d_y, d_x)
        self.yaw_target = self.yaw + (angle * 360 / (2 * 3.141))
        # print(self.yaw)
        # print(angle)
